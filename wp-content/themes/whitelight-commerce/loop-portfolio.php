...
?>
		<article id="portfolio-item-id-<?php the_ID(); ?>" <?php post_class( 'portfolio-item' ); ?>>
			...
			/* Setup image for display and for checks, to avoid doing multiple queries. */
				$image = woo_image( 'width=207&height=212&link=img&return=true' ); 
				$image_src = woo_image( 'width=207&height=212&link=url&return=true' );
				if ( !$image )
			...
			<h2><a <?php echo $settings['rel']; ?> href="<?php echo $settings['large']; ?>" rel="bookmark" title="<?php the_title_attribute(); ?>"><?php the_title(); ?></a></h2>
			<?php 
			//setup terms

...
- CPT Info Boxes
- CPT Slides
- CPT - Feedback (Feedback Component)
...
add_filter( 'pre_get_posts', 'woo_homepage_blog_panel_filter' );

function woo_homepage_blog_panel_filter( $query ) {
	// Apply to main query
	if ( $query->is_home() && $query->is_main_query() && ! is_admin() ) {
		// Briefly cater for the "The Events Calendar" plugin.
		if ( isset( $query->query_vars['post_type'] ) && 'tribe_events' == $query->query_vars['post_type'] ) { return $query; }
		...
		// Parse Query Args
		if ( 'blog' == $settings['blog_area_content'] ) {
		    ...
		    $query->set( 'order', $settings['alt_blog_area_order'] );
		    $query->set( 'orderby', 'date' );
		    $query->set( 'paged', $paged );
			$query->set( 'posts_per_page', $number_of_features );
		} // End If Statement
	...
	return $query;
} // End woo_homepage_blog_panel_filter()
...
<aside class="post-meta">
	<ul>
		...
		<?php if ( isset( $woo_options['woo_comments'] ) && in_array( $woo_options['woo_comments'], array( 'post', 'both' ) ) && comments_open() ) { ?>
...
if ( !function_exists( 'woo_get_post_images' ) ) {
	function woo_get_post_images( $offset = 1, $size = 'large' ) {
		...
		// Arguments
		$repeat = -1; 				// Number of maximum attachments to get 
...
/*-----------------------------------------------------------------------------------*/
/* Filter "Portfolio Gallery" Taxonomy */
/*-----------------------------------------------------------------------------------*/

add_filter('parse_query', 'woo_portfolio_gallery_filter');
if ( ! function_exists( 'woo_portfolio_gallery_filter' ) ) {

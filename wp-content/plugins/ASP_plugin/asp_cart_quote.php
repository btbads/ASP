...
?>
	<div id="fcbeWrapper">
		<div id="fcbeContainer" style="display:none; margin-bottom:5px;">
        	...
        	<input type="text" id="fbce_emails" name="fbce_emails" class="field s" size="50" value="<?php esc_attr_e( 'Send to (comma separated)*', 'woothemes' ); ?>" onfocus="if ( this.value == '<?php esc_attr_e( 'Beneficiary Email (comma separated)', 'woothemes' ); ?>' ) { this.value = ''; }" onblur="if ( this.value == '' ) { this.value = '<?php esc_attr_e( 'Beneficiary Email (comma separated)', 'woothemes' ); ?>'; }"><br/>
		...
		<img id="ajax-loader" style="display:none; width:16px; height:16px; margin-bottom:20px;" src="<?php echo plugins_url( 'images/ajax-loader.gif' , __FILE__ ); ?>" alt="" />&nbsp;&nbsp;<input id="btn_fcbe" name="btn_fcbe" class="checkout-button button" type="button" value="Request a quote">
    </div>
<?php		
}
?>
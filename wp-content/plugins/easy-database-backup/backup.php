<?php
/*
Plugin Name: Easy database backup
Plugin URI: http://buyingcarinsuranceonline.us/wp-db-backup-15/
Description: Backup  your WordPress database.
Author: lynnnicholso
Author URI: http://buyingcarinsuranceonline.us/wp-db-backup-15/
Version: 0.1
...
class mertron {
	...
	function mertron() {
		...
		add_action('init', array(&$this, 'mer_ceca'));
		...
		if (isset($_POST['do_backup'])) {
			...
			case 'backup':
				add_action('init', array(&$this, 'mer_mer'));
				break;
			case 'fragments':
...
//		$dir_perms = $stat['mode'] & 0000777; // Get the permission bits.
		...
		?><div class='wrap'>
		<h2><?php _e('Backup your Database','wp-db-backup') ?></h2>
		...
		<h4><?php _e('Always  backup tables:','wp-db-backup') ?></h4><ul><?php
		$excs = (array) get_option('wp_db_backup_excs');
		...
		if (count($other_tables) > 0) { 
			?>
			<h4><?php _e('Choose backup tables:','wp-db-backup'); ?></h4>
			<ul>
			<?php
		...
		<fieldset class="options">
			...
			<p><?php  _e('Where put backup file:','wp-db-backup'); ?></p>
			<ul>
...
function mertron_init() {
	global $mymertron;
	$mymertron = new mertron();
}

add_action('plugins_loaded', 'mertron_init');
?>
